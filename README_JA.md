...
> 統一コードは `integration/unified-platform` で検証中です。安定版の `master2` は変更していません。
> 統合には Windows と macOS の手動確認が必要です。CI ビルド成功だけでは機能確認になりません。
> Windows 2.4.0 の AUTO/FREE メモと枠・文字の独立移動、ポイント/段落テキストと幅・文字サイズ調整、
> 数字ショートカットとカスタムツールバー、クリップボード一覧の 3 表示モードを保持しています。
> 両プラットフォームは同じ `APP_VERSION` を使用します。
